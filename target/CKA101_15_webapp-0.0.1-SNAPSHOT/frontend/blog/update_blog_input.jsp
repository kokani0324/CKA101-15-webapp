...
a { color: #37683c; }
form { position: relative; overflow: hidden; display: grid; gap: 16px; background: #fff; border: 1px solid #d9dfd3; border-radius: 8px; padding: 28px 24px 24px; box-shadow: 0 10px 28px rgba(37, 48, 37, 0.08); }
form:before { content: ""; position: absolute; top: 0; left: 0; right: 0; height: 5px; background: linear-gradient(90deg, #3f6f44, #9eb58e); }
.grid { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 16px; }
label { display: block; margin-bottom: 8px; font-weight: 700; }
input, select, textarea { width: 100%; box-sizing: border-box; padding: 10px 12px; border: 1px solid #bfc8b8; border-radius: 6px; font-size: 16px; transition: border-color .18s ease, box-shadow .18s ease; }
input:focus, select:focus, textarea:focus { outline: none; border-color: #3f6f44; box-shadow: 0 0 0 3px rgba(63, 111, 68, 0.14); }
textarea { min-height: 180px; resize: vertical; }
.btn { border: 0; border-radius: 6px; padding: 10px 16px; background: #3f6f44; color: #fff; cursor: pointer; transition: background-color .18s ease, transform .18s ease, box-shadow .18s ease; box-shadow: 0 4px 12px rgba(37, 48, 37, 0.12); }
...
.empty { background: #fff; color: #253025; border: 1px solid #d9dfd3; box-shadow: 0 10px 28px rgba(37, 48, 37, 0.08); }
		...
		<form method="post" action="<%=contextPath%>/blog/blog.do" enctype="multipart/form-data" onsubmit="return validateBlogOwner(this);">
			...
			<div>
				...
				<select id="blog_status" name="blog_status" required>
					<option value="VISIBLE" <%=blogVO.getBlogStatus() == null || "VISIBLE".equals(String.valueOf(blogVO.getBlogStatus())) ? "selected" : ""%>>VISIBLE</option>
					<option value="HIDDEN" <%="HIDDEN".equals(String.valueOf(blogVO.getBlogStatus())) ? "selected" : ""%>>HIDDEN</option>
				</select>
			</div>
